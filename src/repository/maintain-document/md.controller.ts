...
export class DocContoller {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('get-maintain-doc')
  async getMaintainDocument(@Res() res: Response) {
    try {
      const data = await this.maintainDocService.getMaintainDocument();
      return res.status(200).json({
        success: true,
        message: 'Maintain document data retrieved successfully.',
        data,
  ...
  async createNarration(
    ...
    try {
      const data = await this.maintainDocService.createReceiptInfo(payload);
      return res.status(200).json({
        success: true,
      ...
      return res.status(500).json({
        success: false,
        message: 'Error retrieving creating receipt data.',
        error: error,
      });
    }
  }
